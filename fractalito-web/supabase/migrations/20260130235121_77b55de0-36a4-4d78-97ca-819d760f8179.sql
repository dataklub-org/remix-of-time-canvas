-- Update the redeem_group_invite_code function:
-- 1. New user should NOT be automatically added to the group (remove that insert)
-- 2. Only add ONE-WAY connection: inviter to new user's circle
-- 3. Remove bidirectional connection (inviter chooses if they want to add back)
...
BEGIN
  ...
  -- NOTE: New user is NOT automatically added to the group
  -- The inviter must manually add them via the group management UI
  
  -- Create notification for the inviter about the new user who used their link
  INSERT INTO public.notifications (user_id, type, title, message, data)
  VALUES (
    v_inviter_user_id,
    'group_invite_used',
    'Someone used your group invite!',
    COALESCE(v_current_username, 'Someone') || ' used your invite link for "' || v_group_name || '". Add them to the group when ready.',
    jsonb_build_object(
